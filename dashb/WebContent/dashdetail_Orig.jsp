...
<%   	String title =  "Olympus FIS Booking Details Report"; 
	...
	sArr = (ArrayList<String>) session.getAttribute("sArr");
	...
	long  daysPend = 0;
		...
		if (dataArr.size() > 0) {
			 for (int k = 0; k < dataArr.size(); k++) {
				...
				//System.out.println("*** ARSZ=" + token_list.length + "--");
...
out.println("<tbody >");
String cells =  buildCells(out, sArr, accessMap); // build data cells from file
out.println(cells);
